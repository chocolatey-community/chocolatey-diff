...
Describe "Test-IsBinary tests" {
    InModuleScope chocolatey-diff {
        Context 'Test-IsBinary Tests' {
            $testCases = @(
                @{ Expected = $false; File = If (![string]::IsNullOrEmpty($env:windir)) { Join-Path -Path $env:windir -ChildPath "win.ini" } else { '/etc/hosts' }; }
                @{ Expected = $true; File = If (![string]::IsNullOrEmpty($env:windir)) { Join-Path -Path $env:windir -ChildPath "notepad.exe" } else { '/bin/cat' }; }
            )
